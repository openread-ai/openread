...
describe('PlanCards', () => {
  ...
  describe('Rendering', () => {
    it('should render all plans', () => {
      ...
      // Use getAllByText since plan name appears in both title and button
    ...
    it('should show plan features', () => {
      render(<PlanCards plans={mockPlans} onUpgrade={mockOnUpgrade} />);
      expect(screen.getByText('Local reading')).toBeTruthy();
  ...
  describe('Current Plan Indication', () => {
    it('should mark the current plan with Current badge', () => {
      render(<PlanCards plans={mockPlans} currentPlanId='plus' onUpgrade={mockOnUpgrade} />);
  ...
  describe('Upgrade Actions', () => {
    it('should show "Switch Plan" button for other plans when user has a plan', () => {
      ...
      // Should have 2 switch buttons (for Plus and Pro)
    ...
    it('should call onUpgrade when clicking upgrade button', async () => {
      ...
      await waitFor(() => {
        expect(mockOnUpgrade).toHaveBeenCalledWith('price_plus_monthly');
      });
    });
    ...
    it('should not call onUpgrade when clicking on free plan', () => {
      ...
      // Find the Free Plan button specifically (it's the one that says "Free Plan" and is a button)
    ...
    it('should show error toast when plan has no productId', async () => {
      const plansWithoutProductId = mockPlans.map((p) => ({
        ...p,
        productId: p.plan === 'plus' ? undefined : p.productId,
      }));
  ...
  describe('Upgrade Restrictions', () => {
    it('should disable upgrade buttons when user is on Pro plan', () => {
      ...
      // Plus Switch Plan button should be disabled
