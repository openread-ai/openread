...
describe('book upload eligibility', () => {
  it.each(predicateCases)(
    ...
    ({ catalogBookId, storagePath, catalogHash, uploadedAt }) => {
      ...
      expect(canUploadToUserCloud(book)).toBe(!expectedRemoteCopy);
    },
  );
  it('allows manual local books that are not deleted and not uploaded', () => {
    expect(canUploadToUserCloud(baseBook())).toBe(true);
  });

  it('excludes deleted and already-uploaded manual books', () => {
    expect(canUploadToUserCloud(baseBook({ deletedAt: Date.now() }))).toBe(false);
    expect(canUploadToUserCloud(baseBook({ uploadedAt: Date.now() }))).toBe(false);
  });
  ...
  it('treats catalog identifiers as catalog-backed and not user-upload eligible', () => {
    ...
    expect(canUploadToUserCloud(book)).toBe(false);
  });
  ...
  it('requires a local file or recoverable source URL for background upload', async () => {
    ...
    await expect(canUploadBookNow(baseBook(), appService)).resolves.toBe(true);
    await expect(canUploadBookNow(baseBook({ title: 'Missing Book' }), appService)).resolves.toBe(
      false,
    );
    await expect(
      canUploadBookNow(
