...
const logger = createLogger('text-translation');
...
export function useTextTranslation(
  ...
  const observeTextNodes = () => {
    ...
    logger.info('Observing text nodes for translation', nodes.length);
  ...
  const translateInRange = useCallback(
    debounce((range: Range) => {
      ...
      if (nodes.length === 0) {
        logger.warn('No text nodes available for translation.');
        return;
      }
      const { startIndex, endIndex } = findNodeIndicesInRange(range, nodes);
      if (startIndex === -1) {
        logger.info('Range not found in text nodes');
        return;
      }
