...
const logger = createLogger('openlibrary');
...
export class OpenLibraryProvider extends BaseMetadataProvider {
  ...
  protected async searchByISBN(isbn: string): Promise<Metadata[]> {
    ...
    try {
      ...
      logger.error('Open Library ISBN search failed:', error);
      throw error;
    }
  }
  ...
  protected async searchByTitle(
    ...
    try {
      ...
      const data = await response.json();
      logger.info('Open Library search results:', data);
      ...
      logger.error('Open Library title search failed:', error);
      throw error;
    }
  }
