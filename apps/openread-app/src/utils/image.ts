 ...
 * - Add Openread icon overlay at bottom right (10px padding)
 * - Return as JPEG blob
 */
export async function processDiscordCover(coverUrl: string, iconUrl: string): Promise<Blob> {
  ...
  try {
    ...
    logger.error('Error processing Discord cover:', error);
    throw error;
  }
}
...
export async function fetchImageAsBase64(
  ...
  try {
    ...
    logger.error('Error fetching and encoding image:', error);
    throw error;
  }
}
