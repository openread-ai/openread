...
const logger = createLogger('foliate-viewer');
...
const FoliateViewer: React.FC<{
  ...
  const readerLayout = viewSettings
    ...
    : null;
  const isContinuousLayout = readerLayout?.layoutMode === 'continuous';
  ...
  const getDocTransformHandler = ({ width, height }: { width: number; height: number }) => {
    return (event: Event) => {
      ...
      detail.data = Promise.resolve(detail.data)
        ...
        .catch((e) => {
          logger.error(`Failed to load ${detail.name}`, e);
          return '';
        });
    };
  };
  ...
  const docLoadHandler = (event: Event) => {
    ...
    const detail = (event as CustomEvent).detail;
    logger.info('doc index loaded', detail.index);
    if (detail.doc) {
      ...
      applyScrollModeClass(detail.doc, isContinuousLayout);
  ...
  const evalInlineScripts = (doc: Document) => {
    if (doc.defaultView && doc.defaultView.frameElement) {
      ...
      scripts.forEach((script, index) => {
        ...
        try {
          logger.warn('Evaluating inline scripts in iframe');
          iframe.contentWindow?.eval(scriptContent);
        } catch (error) {
          logger.error(`Error executing iframe script ${index + 1}`, error);
        }
      });
    }
  };
  ...
  useEffect(() => {
    ...
    const openBook = async () => {
      logger.info('Opening book', bookKey);
      ...
      if (bookDoc.rendition?.layout === 'pre-paginated' && bookDoc.sections) {
        bookDoc.rendition.spread =
          readerLayout?.layoutMode === 'paged' ? viewSettings.pageSpreadMode : 'none';
  ...
  useEffect(() => {
    if (viewRef.current && viewRef.current.renderer) {
      ...
      docs.forEach(({ doc }) => {
        ...
        applyScrollModeClass(doc, isContinuousLayout);
      });
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [
  ...
  const showViewMargins = viewSettings?.showMarginsOnScroll && isContinuousLayout;
