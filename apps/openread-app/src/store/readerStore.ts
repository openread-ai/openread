...
interface ReaderStore {
  ...
  setRSVPEnabled: (key: string, enabled: boolean) => void;
...
export const useReaderStore = create<ReaderStore>((set, get) => ({
  ...
  initViewState: async (
    ...
    try {
      ...
      logger.error('Failed to initialize view state', error);
      set((state) => ({
        viewStates: {
          ...state.viewStates,
          [key]: {
      ...
      return {
        viewStates: {
          ...
          [key]: {
            ...viewState,
            ttsEnabled: enabled,
            // Mutual exclusion: disable RSVP when enabling TTS
            rsvpEnabled: enabled ? false : viewState.rsvpEnabled,
          },
        },
      };
    }),

  setRSVPEnabled: (key: string, enabled: boolean) =>
    set((state) => {
      const viewState = state.viewStates[key];
      if (!viewState) return state;
      return {
        viewStates: {
          ...
          [key]: {
            ...viewState,
            rsvpEnabled: enabled,
            // Mutual exclusion: disable TTS when enabling RSVP
            ttsEnabled: enabled ? false : viewState.ttsEnabled,
          },
        },
      };
    }),
