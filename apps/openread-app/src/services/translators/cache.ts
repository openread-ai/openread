...
const logger = createLogger('translatorCache');
...
const openDatabase = (): Promise<IDBDatabase> => {
  return new Promise((resolve, reject) => {
    if (!window.indexedDB) {
      logger.warn('IndexedDB not supported. Using in-memory cache only.');
...
export const loadCacheFromDB = async (
  ...
  try {
    ...
    if (options.onlyLoadProviders && options.onlyLoadProviders.length > 0) {
      ...
      request.onerror = (event) => {
        logger.error('Error loading cache from IndexedDB:', event);
      };
    }
  ...
  try {
    ...
    return new Promise((resolve, reject) => {
      ...
      transaction.onerror = (event) => {
        logger.error('Error storing in IndexedDB:', event);
...
export const clearCache = async (filter?: CacheFilterOptions): Promise<number> => {
  ...
  try {
    ...
    logger.error('Error clearing IndexedDB cache:', error);
    return deletedCount;
  }
};
...
export const getCacheStats = async (
  ...
  try {
    ...
    logger.error('Error getting IndexedDB stats:', error);
    return { memoryCacheEntries, memoryCacheSizeInBytes };
  }
};
