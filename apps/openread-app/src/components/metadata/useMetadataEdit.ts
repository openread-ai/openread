...
export const useMetadataEdit = (metadata: BookMetadata | null) => {
  ...
  const handleFieldValidation = (field: string, value: string) => {
    ...
    switch (field) {
      ...
      case 'author':
        if (!value.trim()) {
          logger.warn(`Field ${field} cannot be empty`);
      ...
      case 'published':
        if (value.trim()) {
          ...
          if (!validationResult.isValid) {
            logger.warn(`Invalid date for field ${field}:`, validationResult.error);
      ...
      case 'language':
        if (value.trim()) {
          ...
          if (!validationResult.isValid) {
            logger.warn(`Invalid language for field ${field}:`, validationResult.error);
            setFieldErrors((prev) => ({ ...prev, [field]: validationResult.error || '' }));
            return false;
          }
        }
      ...
      case 'subject':
        if (value.trim()) {
          ...
          if (!validationResult.isValid) {
            logger.warn(`Invalid subjects for field ${field}:`, validationResult.error);
            setFieldErrors((prev) => ({ ...prev, [field]: validationResult.error || '' }));
            return false;
          }
        }
  ...
  const handleAutoRetrieve = async () => {
    ...
    try {
      ...
      logger.error('Failed to retrieve metadata:', error);
    } finally {
