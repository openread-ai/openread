...
const logger = createLogger('rsvpUtils');

// Hoisted regex — avoids recompilation on every isCJKPunctuation call
const CJK_PUNCTUATION_RE =
  /[。！？，、；：""''（）《》【】『』「」〈〉〔〕〖〗〘〙〚〛…—～·․‥⋯﹐﹑﹒﹔﹕﹖﹗﹙﹚﹛﹜﹝﹞！＂＃＄％＆＇（）＊＋，－．／：；＜＝＞？＠［＼］＾＿｀｛｜｝～｟｠｡｢｣､･\u3000-\u303F\uFF00-\uFFEF]/;

/**
 * Check if a character is a CJK character.
 * Uses codePointAt to correctly handle supplementary plane characters (Extensions B-E).
 */
...
export function isCJKPunctuation(text: string): boolean {
  return CJK_PUNCTUATION_RE.test(text);
}
...
export function segmentCJKText(text: string): string[] {
  ...
  if (typeof Intl !== 'undefined' && 'Segmenter' in Intl) {
    try {
      ...
      logger.warn('Intl.Segmenter failed, falling back to simple segmentation:', error);
    }
  }
