...
function parseVariableFontAxes(dataView: DataView, fvarTableOffset: number): VariableFontAxis[] {
  try {
    ...
    logger.warn('Failed to parse fvar table:', error);
    return [];
  }
}
  ...
  try {
    ...
    if (os2TableOffset > 0) {
      try {
        ...
        logger.warn('Failed to parse OS/2 table, falling back to style name analysis');
      }
    }
    ...
    logger.warn(`Failed to parse font: ${error}`);
    return {
      name: fallbackName,
      family: fallbackName,
      weight: 400,
