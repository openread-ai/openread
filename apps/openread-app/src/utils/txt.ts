...
export class TxtToEpubConverter {
  public async convert(options: Txt2EpubOptions): Promise<ConversionResult> {
    ...
    try {
      // Unicode property escapes may not be supported in older browsers (Safari < 15)
      matchedAuthor = matchedAuthor.replace(/^[\p{P}\p{S}]+|[\p{P}\p{S}]+$/gu, '');
    } catch (err) {
      // Fallback: keep original matchedAuthor if regex fails (older browser)
      logger.debug('Unicode property escape regex not supported:', err);
    }
  ...
  private detectEncoding(buffer: ArrayBuffer): string | undefined {
    try {
      ...
      logger.info(`UTF-8 validity: ${validPercentage.toFixed(2)}%`);

      // If more than 80% is valid UTF-8, consider it UTF-8 with some corruption
      if (validPercentage > 80) {
        logger.info('Treating as UTF-8 despite some invalid sequences');
        return 'utf-8';
      }
