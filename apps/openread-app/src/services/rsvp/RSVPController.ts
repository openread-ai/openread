...
export class RSVPController extends EventTarget {
  ...
  private loadPositionFromStorage(): RsvpPosition | null {
    ...
    if (stored) {
      try {
        ...
        logger.warn('RSVPController: Corrupted position data, clearing:', err);
  ...
  start(retryCount = 0): void {
    ...
    if (words.length === 0) {
      ...
      logger.warn('RSVPController: No words extracted after retries');
      this.dispatchEvent(
        new CustomEvent('rsvp-error', { detail: 'No content available for RSVP' }),
      );
      return;
    }
  ...
  loadNextPageContent(retryCount = 0): void {
    ...
    if (words.length === 0) {
      ...
      logger.warn('RSVPController: No content found for next page after retries');
      this.dispatchEvent(
        new CustomEvent('rsvp-error', { detail: 'No content available for next page' }),
      );
      this.pause();
      return;
    }
      ...
      if (node.nodeType === Node.TEXT_NODE) {
        ...
        for (const word of nodeWords) {
          ...
          try {
            ...
            logger.warn('RSVPController: Failed to create range for word:', { word, err });
            words.push({
              text: word,
              orpIndex: this.calculateORP(word),
              pauseMultiplier: this.getPauseMultiplier(word),
