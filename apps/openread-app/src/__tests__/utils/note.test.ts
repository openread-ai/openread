...
describe('renderNoteTemplate', () => {
  const sampleData: NoteTemplateData = {
    ...
    exportDate: 1705276800000, // 2024-01-15 00:00:00 UTC
    chapters: [
      {
        title: 'Chapter 1',
        annotations: [
...
{% endif %}{% endfor %}{% endfor %}`;
  ...
  describe('Date filter', () => {
    ...
    it('should handle string date input', () => {
      // Use midday UTC timestamp to avoid timezone edge cases
      const middayData: NoteTemplateData = {
        ...sampleData,
        exportDate: 1705320000000, // 2024-01-15 12:00:00 UTC
      };
      const template = "{{ exportDate | date('%Y-%m-%d') }}";
      const result = renderNoteTemplate(template, middayData);
      expect(result).toBe('2024-01-15');
    });
  ...
  describe('Chained filters', () => {
    ...
    it('should chain date and string filters', () => {
      ...
      const template = "{{ exportDate | date('%b %d, %Y') | upper }}";
      const result = renderNoteTemplate(template, middayData);
