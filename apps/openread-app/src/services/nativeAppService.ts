...
const logger = createLogger('nativeAppService');

declare global {
  interface Window {
    __OPENREAD_IS_EINK?: boolean;
    __OPENREAD_IS_APPIMAGE?: boolean;
    __OPENREAD_UPDATER_DISABLED?: boolean;
  }
}
...
export const nativeFileSystem: FileSystem = {
  ...
  async openFile(path: string, base: BaseDir, name?: string) {
    ...
    if (isValidURL(path)) {
      ...
      if (path.includes('com.android.externalstorage')) {
        ...
        if (!res.success) {
          logger.error('Failed to open file:', res);
  ...
  async copyFile(srcPath: string, dstPath: string, base: BaseDir) {
    try {
      ...
      logger.info('Failed to create directory for copying file:', error);
    }
    if (isContentURI(srcPath)) {
      ...
      if (!res.success) {
        logger.error('Failed to copy file:', res);
  ...
  async readDir(path: string, base: BaseDir) {
    ...
    if (!baseDir || baseDir === 0) {
      try {
        ...
        logger.error('Rust read_dir failed, falling back to JS recursion', e);
      }
    }
      ...
      for (const entry of entries) {
        if (entry.isDirectory) {
          ...
          try {
            ...
            logger.warn(`Skipping unreadable dir: ${dir}`);
          }
        } else {
...
const DIST_CHANNEL = (process.env['NEXT_PUBLIC_DIST_CHANNEL'] || 'openread') as DistChannel;
...
export class NativeAppService extends BaseAppService {
  ...
  override isAppImage = Boolean(window.__OPENREAD_IS_APPIMAGE);
  override isEink = Boolean(window.__OPENREAD_IS_EINK);
  ...
  override hasUpdater =
    ...
    !window.__OPENREAD_UPDATER_DISABLED;
    ...
    try {
      ...
      if (lastMigrationVersion < 20251029) {
        try {
          ...
          logger.error('Error migrating to version 20251029:', error);
        }
      }
  ...
  async migrate20251029() {
    logger.info('Running migration 20251029 to update paths in Images dir...');
    const rootPath = await this.resolveFilePath('..', 'Data');
    const newDir = await this.fs.getPrefix('Images');
    const oldDir = await join(rootPath, 'Images', 'Openread', 'Images');

    const oldDirExists = await this.exists(oldDir, 'None');
    if (!oldDirExists) {
      logger.info('Migration 20251029: old Images dir not found, skipping');
      return;
    }

    await copyFiles(this, oldDir, newDir);

    const dirToDelete = await join(rootPath, 'Images', 'Openread');
