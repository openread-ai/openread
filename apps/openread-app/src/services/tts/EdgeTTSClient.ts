...
const logger = createLogger('edgeTTS');
    ...
    if (preload) {
      ...
      for (let i = 0; i < Math.min(maxImmediate, marks.length); i++) {
        ...
        await this.#edgeTTS
          ...
          .catch((err) => {
            logger.warn('Error preloading mark', { index: i, err });
          });
      }
        ...
        (async () => {
          for (let i = maxImmediate; i < marks.length; i++) {
            ...
            try {
              ...
              logger.warn('Error preloading mark (bg)', { index: i, err });
            }
          }
        })();
      }
    ...
    for (const mark of marks) {
      ...
      try {
        ...
        if (error instanceof Error && error.message === 'No audio data received.') {
          logger.warn('No audio data received for:', mark.text);
          yield { code: 'end', message: `Chunk finished: ${mark.name}` } as TTSMessageEvent;
          continue;
        }
        const message = error instanceof Error ? error.message : String(error);
        logger.warn('TTS error for mark:', { text: mark.text, message });
