...
const logger = createLogger('kosync');
...
export class KOSyncClient {
  ...
  async connect(
    ...
    try {
      ...
      logger.error('KOSync connection failed', e);
  ...
  async getProgress(book: Book): Promise<KoSyncProgress | null> {
    ...
    try {
      ...
      logger.error('KOSync getProgress failed', e);
      return null;
    }
  }
  ...
  async updateProgress(book: Book, progress: string, percentage: number): Promise<boolean> {
    ...
    try {
      ...
      logger.error('KOSync updateProgress failed', e);
      return false;
    }
  }
  ...
  getDocumentDigest(book: Book): string {
    if (this.config.checksumMethod === 'filename') {
      logger.warn('This is not possible anymore, using md5 instead.');
    }
    return book.hash;
  }
}
