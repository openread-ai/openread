...
const RSVPControl: React.FC<RSVPControlProps> = ({ bookKey }) => {
  ...
  const removeRsvpHighlight = useCallback(() => {
    ...
    if (tempHighlightRef.current && view) {
      try {
        ...
        logger.warn('Failed to remove RSVP highlight', err);
      }
    }
  ...
  const handleStart = useCallback(
    (selectionText?: string) => {
      ...
      if (!controllerRef.current) {
        const ctrl = new RSVPController(view, bookKey);
        ctrl.addEventListener('rsvp-error', (e: Event) => {
          const message = (e as CustomEvent<string>).detail;
          eventDispatcher.dispatch('toast', { message, type: 'warning' });
        });
      ...
      const handleRsvpStop = (e: Event) => {
        ...
        if (stopPosition && stopPosition.cfi) {
          try {
            ...
            logger.warn('Failed to sync RSVP position', err);
          }
        }
      };
  ...
  const handleRequestNextPage = useCallback(async () => {
    ...
    try {
      ...
      logger.warn('Failed to navigate to next section', err);
      controllerRef.current?.pause();
      eventDispatcher.dispatch('toast', {
        message: _('Failed to load next chapter'),
        type: 'warning',
      });
      return;
    }
  ...
  }, [_, bookKey, getProgress, getView, removeRsvpHighlight]);
