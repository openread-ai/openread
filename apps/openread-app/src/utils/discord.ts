 ...
 * - Processes cover with Openread icon overlay
 */
const getCoverUrlForDiscord = async (
  ...
  try {
    ...
    logger.info('Processed Discord cover for book:', book.title);
    ...
    logger.warn('Failed to process/upload cover for Discord:', error);
    coverUrlCache.set(book.hash, { url: null, timestamp: Date.now() });
    return undefined;
  }
};
