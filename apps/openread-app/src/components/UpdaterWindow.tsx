  ...
  useEffect(() => {
    ...
    const checkAndroidUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const apkFilePath = await appService.resolveFilePath(
          `Openread_${data.version}_${arch}.apk`,
          'Cache',
        );
        setUpdate({
          currentVersion,
          ...
          downloadAndInstall: async (onEvent) => {
            await new Promise<void>(async (resolve, reject) => {
              ...
              await tauriDownload(downloadUrl, apkFilePath, (progress) => {
                ...
                if (!total && progress.total) {
                  ...
                  logger.info('APK downloaded to', apkFilePath);
            ...
            if (res.success) {
              ...
              logger.error('Failed to install APK:', res.error);
            }
          },
        } as GenericUpdate);
      }
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        setUpdate({
          ...
          downloadAndInstall: async (onEvent) => {
            ...
            try {
              logger.info('Launching new executable:', exeFilePath);
              const command = Command.create('start-openread', ['/C', 'start', '', exeFilePath]);
              await command.spawn();
              logger.info('New executable launched, exiting current app...');
              setTimeout(async () => {
                await exit(0);
              }, 500);
            } catch (error) {
              logger.error('Failed to launch new executable:', error);
            }
          },
        } as GenericUpdate);
      }
    ...
    const checkAppImageUpdate = async () => {
      ...
      if (semver.gt(data.version, currentVersion)) {
        ...
        const downloadUrl = data.platforms[platformKey]?.url as string;
        const appImageFileName = `Openread_${data.version}_${arch}.AppImage`;
        ...
        setUpdate({
          ...
          downloadAndInstall: async (onEvent) => {
            ...
            try {
              ...
              logger.info('AppImage made executable:', appImageFilePath);

              // Launch the new AppImage
              logger.info('Launching new AppImage:', appImageFilePath);
              const launchCommand = Command.create('launch-appimage', [appImageFilePath]);
              await launchCommand.spawn();
              logger.info('New AppImage launched, exiting current app...');
              setTimeout(async () => {
                await exit(0);
              }, 500);
            } catch (error) {
              logger.error('Failed to launch new AppImage:', error);
            }
          },
        } as GenericUpdate);
      }
      ...
      if (!targetLang.toLowerCase().startsWith('en')) {
        for (const entry of changelogs) {
          try {
            ...
            logger.info('Failed to translate changelog:', error);
          }
        }
      }
      ...
      switch (event.event) {
        ...
        case 'Progress':
          ...
          if (downloaded - lastLogged >= 1 * 1024 * 1024) {
            logger.info(`downloaded ${downloaded} bytes from ${contentLength}`);
          ...
          {checkUpdate ? (
            <div className='text-base-content flex-grow text-sm'>
              <h2 className='mb-4 text-center font-bold sm:text-start'>
                {_('A new version of Openread is available!')}
              </h2>
              <p className='mb-2'>
                {_('Openread {{newVersion}} is available (installed version {{currentVersion}}).', {
                  newVersion,
                  currentVersion,
                })}
              </p>
...
export const UpdaterWindow = () => {
  ...
  return (
    <Dialog
      id='updater_window'
      isOpen={isOpen}
      title={checkUpdate ? _('Software Update') : _("What's New in Openread")}
